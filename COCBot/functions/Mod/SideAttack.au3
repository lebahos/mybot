Func LaunchSideAttack($listInfoDeploy, $CC, $King, $Queen, $Warden, $overrideSmartDeploy = -1)
	...
	Local $barPosition = -1, $spellCount = 0, $unitCount = 0
		...
		If $kind <> $DeDeployEmptyString And $barPosition <> -1 Then
			Switch $kind
				...
				Case $eRSpell, $eHSpell, $eJSpell, $eHaSpell, $eFSpell, $ePSpell
					...
					EndIf
				Case $eESpell
					If $CurEarthSpell >= 4 Then ;4 quakes or go home
					...
					Else
						If $CurEarthSpell > 0 and $CurEarthSpell < 4 Then SetLog("Only " & $CurEarthSpell & " " & getTranslatedTroopName($kind) & " available.  Waiting for 4.")
					EndIf
					; $debugSetlog = 0
				Case Else
					...
					If $unitCount > 0 Then
						If $positionside <> "L" and $positionSide <> "R" Then
							If LauchTroop($kind, $listInfoDeploy[$i][1], $listInfoDeploy[$i][2], $listInfoDeploy[$i][3], $position, $overrideSmartDeploy) Then
